...
class Solution:
    def updateMatrix(self, matrix: List[List[int]]) -> List[List[int]]:
        d = [[sys.maxsize for _ in range(len(matrix[0]))] for _ in range(len(matrix))]
        ...
        for i in range(len(matrix)-1,-1,-1):
            for j in range(len(matrix[0])-1,-1,-1):
                if matrix[i][j]==0: d[i][j]==0
                    ...
                    elif i==len(matrix)-1: d[i][j]=min(d[i][j],d[i][j+1]+1)
                    elif j==len(matrix[0])-1: d[i][j]=min(d[i][j],d[i+1][j]+1)
                    else: d[i][j]=min(d[i][j],d[i+1][j],d[i][j+1])+1 
        
        return d
#### Solution on 1/31, why doesn't it work?